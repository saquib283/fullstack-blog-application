...
// import {CommentType,UserForComment} from "@/types/blog"
...
//     thumbnail?: { url: string; alt?: string; caption?: string };
//     content: string | { time: number; blocks: any[]; version: string; };
//     metaTitle?: string;
...
//     const { data, loading, error } = useFetch<{ blog: Blog }>(
...
//         toast.success('Thanks for subscribing! Stay tuned for more updates.');
//     };
...
//                     <button
//                         onClick={handleLike}
//                         className={`flex items-center gap-2 px-4 py-2 rounded-md text-lg font-semibold transition-all duration-300
//                             ${isLiked
//                                 ? 'bg-red-500 text-white shadow-md'
//                                 : 'bg-gray-100 text-gray-700 hover:bg-red-100 hover:text-red-600 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-red-900 dark:hover:text-red-100'
//                             }`}
//                     >
//                         <Heart size={20} fill={isLiked ? 'white' : 'currentColor'} strokeWidth={isLiked ? 0 : 2} />
...
//                 <button
//                     onClick={handleShare}
//                     className="flex items-center gap-2 px-4 py-2 rounded-md text-lg font-semibold bg-gray-100 text-gray-700 hover:bg-blue-100 hover:text-blue-600 transition-all duration-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-blue-900 dark:hover:text-blue-100"
//                 >
//                     <Share2 size={20} />
...
//                     className="flex items-center gap-2 px-4 py-2 rounded-md text-lg font-semibold bg-red-600 hover:bg-red-700 cursor-pointer"
//                 >
//                     <BellDot size={20} />
//                     <span>Subscribe</span>
//                 </button>
...
// // Make sure the path is correct based on where `blog.ts` is.
// // If it's in `src/types/blog.ts`, then `@/types/blog` is correct.
// import { BlogType, CommentType } from "@/types/blog"; // You might not even need CommentType here if blog.comments is already typed by BlogType
...
//     // Ensure useFetch is typed correctly to return { blog: BlogType }
//     // This assumes your API response wraps the blog data in an object with a 'blog' key
...
//         if (data?.blog?.content) {
//             try {
//                 // Ensure content is parsed correctly from string if it comes as JSON string
//                 const contentObj = typeof data.blog.content === 'string'
//                     ? JSON.parse(data.blog.content)
//                     : data.blog.content;
//                 setParsedBlogContent(contentObj);
...
//         }
//         if (data?.blog) {
//             // Access likes and shares from the consistent BlogType
//             setLikeCount(data.blog.likes?.length || 0);
//             setShareCount(data.blog.shares?.length || 0);
//             setIsLiked(false); // You might want to check if the current user has liked it here
//         }
//     }, [data]);
...
//             <CommentSection
//                 blogId={blog._id}
//                 // Now, blog.comments should correctly match CommentType[] from your types/blog.ts
//                 initialComments={blog.comments || []}
//                 allowComments={blog.allowComments || false}
//             />
//         </div>
